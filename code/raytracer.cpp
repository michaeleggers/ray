...
// TODO(Michael): figure out why the image is flipped along the y-axis. -> see
// below where viewSide is computed!
...
void createRandomScene(int hitableCount, Hitable** hitables, int materialCount, Material** materials)
{
    *materials         = (Material*)malloc(sizeof(Material) * materialCount);
    ...
    for ( int i = 0; i < materialCount; ++i )
    {
        ...
        material->attenuation = { randBetween(0.3f, 1.0f), randBetween(0.3f, 1.0f), randBetween(0.3f, 1.0f) };
    ...
    *hitables        = (Hitable*)malloc(sizeof(Hitable) * hitableCount);
    ...
    for ( int i = 0; i < hitableCount; ++i )
    {
        ...
        sphere.sphere = { { randBetween(-40.0f, 40.0f), randBetween(-20.0f, 20.0f), randBetween(-20.0f, 20.0f) },
...
bool hitSphere(v3 rayOrigin, v3 rayDirection, Sphere object, HitRecord* hitrec)
{
    ...
    float c            = dot(rayOriginToSphereCenter, rayOriginToSphereCenter) - object.radius * object.radius;
    ...
    else
    {
        ...
        if ( distance > 0.001f ) hasHit = true;
    }
...
v3 randomInUnitSphere()
{
    ...
    do
    {
        test = { rand() / float(RAND_MAX), rand() / float(RAND_MAX), rand() / float(RAND_MAX) };
        p    = 2.0f * test - one;
    } while ( dot(p, p) >= 1.0f );
    return p;
}
...
int main(int argc, char** argv)
{
    ...
    if ( argc < 4 )
    {
        printf("Usage: raytracer <samples per pixel> <resolution x> <resolution y>\n");
        exit(1);
    }
    int sampleCount = atoi(argv[ 1 ]);
    resolutionX     = atoi(argv[ 2 ]);
    resolutionY     = atoi(argv[ 3 ]);
    ...
    char*       outputBuffer
        = (char*)malloc(sizeof(char) * 3 * 4 * (resolutionX * resolutionY) + sizeof(char) * ppmHeader.length());
    if ( !outputBuffer )
    {
        return -1;
    }
    ...
    // compute view, right and up vectors via Gram-Schmidt orthogonalization (p.
    // 236 Van Verth)
    ...
    v3 viewSide          = cross(viewDir,
                        viewUp); // TODO(Michael): righ or left handed system? FIGURE THIS OUT!!!!
    ...
    lightsphere1.sphere   = { { 1, 55, 5 }, 20.0f };
    ...
    Hitable scene[] = { testsphere1, testsphere2, testsphere3, testsphere4, lightsphere1 };
    ...
    for ( int row = 0; row < resolutionY; ++row )
    {
        ...
        for ( int col = 0; col < resolutionX; ++col )
        {
            ...
            for ( int i = 0; i < sampleCount; i++ )
            {
                ...
                // NOTE(Michael): -viewportY, because relative viewport Y goes from
                // top(-) to bottom(+)
