...
struct v3 {
  ...
  v3 operator-(v3 b) {
    return {components[0] - b[0], components[1] - b[1], components[2] - b[2]};
  }
  ...
  v3 operator/(float scalar) {
    return {components[0] / scalar, components[1] / scalar,
            components[2] / scalar};
  }

  v3 operator*(const float &scalar) {
    return {components[0] * scalar, components[1] * scalar,
            components[2] * scalar};
  }

  v3 operator*(v3 rhs) {
    return {components[0] * rhs[0], components[1] * rhs[1],
            components[2] * rhs[2]};
  }
};

v3 operator-(const v3 &v) {
  return {-v.components[0], -v.components[1], -v.components[2]};
}

v3 operator*(const float &scalar, const v3 &v) {
...
enum Shading_t { LAMBERT, METAL, DIALECTRIC, DIFFUSE_LIGHT, NORMALS };

enum Geometry_t { SPHERE, TRIANGLE };
...
// function prototypes
bool hit(v3 rayOrigin, v3 rayDirection, Hitable *hitables, int hitableCount,
         HitRecord *hitrec);
